...
async def detect_frame(
    ...
    try:
        ...
        behaviour_alerts = detect_behaviour(yolo_result)
        alerts.extend(behaviour_alerts)
        ...
        if len(violence_frames) >= 16:
            violence_conf = violence_detector.predict(violence_frames[-16:])
        ...
        # ---------------- CHASE Detection ----------------
        chase_result = chase_detector.update(frame, persons)
        if chase_result["chasing"]:
            alerts.append("chase")

        # ---------------- AUDIO Detection ----------------
        if audio_detector.keyword_detected():
            alerts.append("audio_help")
        ...
        for alert in alerts:
            detection_type = mapping.get(alert, alert)
            send_alert(
                {
                    "camera_id": camera_id,
